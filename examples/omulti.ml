...
let loop_select hs mt =
  ...
  let on_event fd event =
    if M.action mt fd event = 0 then mutexify m (fun () -> is_done := true) ()
  in
  M.set_socket_function mt (fun fd what ->
      A.iter
        (fun (h, url) ->
          if Some fd = Curl.get_activesocket h then
            pr "Waiting for event on socket for url %s" url)
        hs;
      mutexify m
        (fun () ->
  ...
  let rec process () =
    M.action_timeout mt;
    let is_done = mutexify m (fun () -> !is_done) () in
    if not is_done then (
      let timeout = if !in_fd = [] && !out_fd = [] then 0.1 else -1. in
    ...
    let args =
      Arg.align
        [
          ("-n", Arg.Set_int n, "<N> ");
          ( "-i",
            Arg.String (fun s -> urls := input_lines s @ !urls),
          ...
          ( "-m",
            ...
            "<wait|select> loop method" );
        ]
    in
    Arg.parse args failwith "Options :";
    match !urls with [url] -> A.make !n url | l -> A.of_list l
  in
